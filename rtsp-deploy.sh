#!/usr/bin/env bash
#
# rtsp-deploy.sh — unattended install of RTSP viewer + autologin + xinit
set -e -o pipefail
...
for set in 1 2 3; do
  echo
  read -rp "▶ Press [Enter] to begin set #$set…" _
...
# 8) Configure real autologin on tty1
echo "Configuring auto-login on tty1…"
mkdir -p /etc/systemd/system/getty@tty1.service.d
cat > /etc/systemd/system/getty@tty1.service.d/autologin.conf << 'DROPIN'
[Service]
ExecStart=
